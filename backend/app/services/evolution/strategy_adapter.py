...
class StrategyAdapter:
    ...
    async def optimize_strategies(self) -> List[dict]:
        """
        ...
        for source in sources:
            change_info = await self._optimize_source(source)
            if change_info:
    ...
    async def _optimize_source(self, source: InformationSource) -> dict | None:
        """
        Analyze a single source and update its config if needed.
        Returns change info dict if changed, else None.
        """
        ...
        new_interval = original_interval
        reason = ""
        ...
        if failed_jobs / total_jobs >= app_settings.STRATEGY_FAILURE_THRESHOLD:
            ...
            reason = f"High failure rate ({failed_jobs}/{total_jobs})"
        ...
        # Check content freshness (only for completed jobs)
            ...
            if completed_jobs:
                ...
                if total_fetched > 0 and total_new == 0:
                    ...
                    reason = "Low content freshness (no new items)"

                # Rule 3: High Freshness -> Speed up
                elif total_fetched > 0 and (total_new / total_fetched) > app_settings.STRATEGY_FRESHNESS_THRESHOLD:
                    new_interval = max(source.check_interval * app_settings.STRATEGY_SPEEDUP_FACTOR, self.MIN_INTERVAL)
                    reason = "High content freshness"

        if int(new_interval) != original_interval:
            logger.info(f"Adjusting source {source.name}: {original_interval} -> {int(new_interval)} ({reason})")
