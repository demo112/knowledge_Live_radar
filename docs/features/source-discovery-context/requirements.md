...
- [ ] AC1: 全结构语境提取
  ...
  - **When**: 触发信息源发现任务
  - **Then**: 系统能够提取金字塔的**全量结构上下文**（Full Structure Context），包括名称、描述、所有层级的节点及其关系
  - **Note**: 需采用文本化序列化（Textual Serialization）或其他 Token 高效的方式，尽可能保留完整的层级结构，**不预设深度限制**（如 L2-L4），而是由 AI 根据内容判断重要性

- [ ] AC2: 智能自适应查询生成
  - **Given**: 提取到的金字塔全量结构上下文
  - **When**: 生成搜索查询
  - **Then**: 系统基于 AI 分析，**灵活生成**多维度的搜索查询策略：
    - **宏观（Macro）**: 针对金字塔整体领域或核心概念（如 "{Domain} 官方文档", "{Concept} 最佳实践"）
    - **中观（Meso）**: 针对关键分支或子领域（如 "{Sub-domain} 架构设计", "{Topic} 深度解析"）
    - **微观（Micro）**: 针对特定高价值节点、新兴技术点或具体实体（如 "{Entity} 最新评测", "{Tech} 教程"）
  - **Constraints**: AI 应自主判断哪些节点值得关注，而非机械地遍历所有节点或仅关注特定层级
