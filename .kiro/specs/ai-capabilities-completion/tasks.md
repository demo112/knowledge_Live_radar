...
- [ ] 20. 实现前端快照历史界面
  ...
  - [ ] 20.6 编写前端单元测试
    - ⚠️ 跳过：缺少前端单元测试基础设施 (Jest/Vitest)
    - 替代方案：在 Task 22.5 中编写 E2E 测试覆盖此功能
...
- [x] 22. 端到端测试和文档
  ...
  - [x] 22.2 编写端到端测试场景 2：内容自动分类
    - 注入测试内容
    - 触发自动分类
    - 验证内容归类到金字塔节点
    - _需求：验收场景 2_
  
  - [x] 22.3 编写端到端测试场景 3：批量处理历史内容
    - 调用批量分类 API
    - 验证任务启动
    - 等待任务完成
    - 验证处理结果
