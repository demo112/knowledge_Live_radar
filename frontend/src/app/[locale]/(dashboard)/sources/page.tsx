...
export default function SourcesPage() {
  ...
  const handleDiscover = async () => {
    setDiscovering(true);
    try {
      // Trigger discovery for all pyramids (no pyramid_id passed)
      const res = await sourceApi.discover();
      if (res.success) {
        alert(t('discovery.started', { count: res.data.count }));
        // Switch to discovery tab to see results
